...
/**
* @class i2cInterface
* @brief helper class for managing i2c
*/
class i2cInterface
{
public:
...
* @brief Class responsible for handling all sensors
*
*/
class SensorsManager
{
  SensorsManager();  //singleton model
  ...
  /* Params calculated during callibration, offset and gain */
  double m_acc_params[3][2];
  double m_gyro_params[3][2];
  double m_compass_params[3][2];
public:


  /// Run one point callibration to calculate offset (and gain for acc). Note: temporarily commented out and hardcoded
  ...
  ///Update readings given current time
