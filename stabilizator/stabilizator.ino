...
void loop()
{
  ...
  // Wypisywanie kata - trzeba sprawdzic na ile to dziala
  const double * eulers = rf.getAngle();
  for(int i = 0; i < 3; ++i)
  {
  ...
  // Wypisywanie predkosci katowej - trzeba sprawdzic na ile to dziala, jakie osie i jednostki
  const double * angleAcc = rf.getAngleAcceleration();
  for(int i = 0; i < 3; ++i)
  {
    Serial.print(angleAcc[i]);
    Serial.print("; ");
  }
  Serial.println();

// PID wylaczony az nie bedzie dobrych katow
/*
   // === Calkuj blad === //
   E_x_sum += error[0];
   E_y_sum += error[1];
