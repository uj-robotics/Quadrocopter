...
void ADXL345Sensor::get_readings(double * data)
{
  ...
  int tmp=0;
  short int tmp2=0;
  // note: scale is included in SensorsManager class
  for (int i=0;i<3;++i)
  {
    tmp2=((int)bytes[2*i] + (((int)bytes[2*i + 1]) << 8));
    data[i]=(double)tmp2;
  }
...
///Update readings : note that sensors are sampled when new reading is available. Sensors on this particular quadrocopter are much faster than sampling rate so it is not needed
